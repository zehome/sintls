...
// rootPath:
//
//     ~/.config/sintls/accounts/
//          │              └── root accounts directory
//          └── "path" option
//
// rootUserPath:
//
//     ~/.config/sintls/accounts/localhost_14000/hubert@hubert.com/
//          │            │             │             └── userID ("email" option)
...
// keysPath:
//
//     ~/.config/sintls/accounts/localhost_14000/hubert@hubert.com/keys/
//          │              │             │             │           └── root keys directory
//          │              │             │             └── userID ("email" option)
//          │              │             └── CA server ("server" option)
//          │              └── root accounts directory
...
// accountFilePath:
//
//     ~/.config/sintls/accounts/localhost_14000/hubert@hubert.com/account.json
//          │            │             │             │             └── account file
//          │            │             │             └── userID ("email" option)
//          │            │             └── CA server ("server" option)
//          │            └── root accounts directory
...
func NewAccountsStorage(ctx *cli.Context) *AccountsStorage {
	...
	serverURL, err := url.Parse(ctx.GlobalString("ca-server"))
	if err != nil {
		log.Fatal(err)
	}
...
func tryRecoverRegistration(ctx *cli.Context, privateKey crypto.PrivateKey) (*registration.Resource, error) {
	...
	config.CADirURL = ctx.GlobalString("ca-server")
	config.UserAgent = fmt.Sprintf("sintls-cli/%s", ctx.App.Version)
